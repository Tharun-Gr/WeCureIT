...
//    @Query("SELECT ws.doctor FROM WorkSchedule ws WHERE ws.facility.id = :facilityId AND ws.workDate = :date AND :time BETWEEN ws.workStartTime AND ws.workEndTime")
    ...
    @Query("SELECT DISTINCT w.facility FROM WorkSchedule w WHERE w.doctor.id = :doctorId AND w.workDate = :date AND :time BETWEEN w.workStartTime AND w.workEndTime")
    List<Facility> findFacilitiesByDoctorIdAndDateTime(@Param("doctorId") int doctorId, @Param("date") Date date, @Param("time") Time time);
}
